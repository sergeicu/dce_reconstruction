...
%
    ...
    % starting point
%     if size(x0,1) == 448
%         x=x0(448/4 + [1:224],448/4 + [1:224],:);    % cropping the image manually. Good job coding!
%     else
        x = x0;
%     end
...
function res = objective(x,dx,t,param) %**********************************
    ...
    % prealocate bigger image
    paddedX = (x+t*dx);
    
    % L2-norm part
    w=param.E*paddedX-param.y;
    L2Obj=w(:)'*w(:);
    ...
    % objective function
    res=L2Obj+param.lambda*L1Obj+param.lambda2*LTIObj;
...
function g = grad(x,param)%***********************************************
    ...
    % LTI regu part
    if param.lambda2
       LTIGrad = 2*(param.W2*x); 
    else
        LTIGrad=0;
    end

    % composite gradient
    g = L2Grad + param.lambda*L1Grad + param.lambda2*LTIGrad;
