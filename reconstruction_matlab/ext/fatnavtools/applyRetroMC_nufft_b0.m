function [gdata, st, newData] = applyRetroMC_nufft_b0(kdata_in, alignMats, alignDim, alignIndices, useTable, dataDims_mm, fullMatrixDims, kspaceCentre, f0_coeffs, echo_time, cgIterations, oversampFactor)
...
% Added: 'f0_coeffs' should be a 4xnT matrix of Z0, Z1, X1, Y1 in Hz 
%        'echo_time' in s
...
[kx, ky, kz] = ndgrid( [1:nx]-kspaceCentre(1), [1:ny]-kspaceCentre(2), [1:nz]-kspaceCentre(3) ); % grid of sampled k-space locs
...
for iT = 1:Nt
    ...
    newVec = (alignMats(1:3,1:3,iT) * [kx(iLine) ky(iLine) kz(iLine)].' ).'; % kx,ky,kz scaled between -1 and 1
    
    % ADDED shift term to account for linear shim terms, x and y permuted??
   % dkz = f0_coeffs(2,iT)*echo_time/Nz/2/1000; % k-space scaled -1 to 1
   % dkx = f0_coeffs(4,iT)*echo_time/Nx/2/1000;
   % dky = f0_coeffs(3,iT)*echo_time/Ny/2/1000; % because co-ordinates in mm
   
   %dkz = 2*pi*f0_coeffs(2,iT)*echo_time/dataDims_mm(3)/1000; % k-space scaled -1/res to 1/res
   %dkx = 2*pi*f0_coeffs(4,iT)*echo_time/dataDims_mm(1)/1000;
   %dky = 2*pi*f0_coeffs(3,iT)*echo_time/dataDims_mm(2)/1000; % because co-ordinates in 1/mm
    
    dkz = 2*f0_coeffs(2,iT)*echo_time/1000; % k-space scaled -1/res to 1/res
    dkx = 2*f0_coeffs(4,iT)*echo_time/1000;
    dky = 2*f0_coeffs(3,iT)*echo_time/1000; % because co-ordinates in 1/mm
   
   
    newVecShift = newVec; 
    newVecShift(:,1) = newVec(:,1) + dkx; 
    newVecShift(:,2) = newVec(:,2) - dky; % was -
    newVecShift(:,3) = newVec(:,3) + dkz; 
    
    nkx(iLine) = newVecShift(:,1); nky(iLine) = newVecShift(:,2); nkz(iLine) = newVecShift(:,3);
    ...
    % ADDED phase correction term to account for delta b0
    newData(iLine) = newData(iLine).*exp(-1i*2*pi*f0_coeffs(1,iT)*echo_time); 
    fprintf('.');
end
fprintf('\n');
