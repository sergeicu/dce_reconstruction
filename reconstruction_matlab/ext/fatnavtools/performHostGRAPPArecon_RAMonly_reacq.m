function [grappaRecon_1DFFT, mOutGRAPPA] = performHostGRAPPArecon_RAMonly_reacq(twix_obj, twix_obj2, reacq_mask, iRepToRecon, iEcoToRecon, nSliceNeighbours)
%
...
% reacq_mask = 1:length(allSamp)
allSamp = unique(twix_obj.imageWithRefscan.Lin);
imageSamp = unique(twix_obj.image.Lin);
linesInImage=zeros(1,length(imageSamp));
for iS = 1:length(imageSamp)
    linesInImage(iS) = find(allSamp==imageSamp(iS));
end
%{
acsSamp = unique(twix_obj.refscan.Lin);
linesInACS = zeros(1,length(acsSamp));
for iS = 1:length(acsSamp)
    linesInACS(iS) = find(allSamp==acsSamp(iS));
end
%}
% Reacquire bad data
reacqData = twix_obj2.image(:,:,:,:,:,:,:,iEcoToRecon,iRepToRecon,:);
linesToReplace = imageSamp(reacq_mask(linesInImage)==1);
allData(:,:,linesToReplace,:,:,:,:,:,:,:) = reacqData(:,:,linesToReplace,:,:,:,:,:,:,:);
...
% Replace GRAPPA data too...
ACSdata = twix_obj.refscan(:,:,:,:,:,:,:,iEcoToRecon,iRepToRecon,:);
% Don't replace GRAPPA data as we are doing this BEFORE motion correction
%acsLinesToReplace = acsSamp(reacq_mask(linesInACS)==1);
%acsLinesToReplace = acsLinesToReplace-acsSamp(1)+1;
%ACSdata(:,:,acsLinesToReplace,:,:,:,:,:,:,:) = reacqData(:,:,acsLinesToReplace,:,:,:,:,:,:,:);
