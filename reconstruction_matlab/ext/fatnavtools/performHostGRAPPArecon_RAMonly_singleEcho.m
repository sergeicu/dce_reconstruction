function [grappaRecon_1DFFT, mOutGRAPPA] = performHostGRAPPArecon_RAMonly_singleEcho(twix_obj, iRepToRecon, iEcoToRecon, nSliceNeighbours)
%
...
tic
grappaRecon_1DFFT = complex(zeros(nread,nc,npe1,npe2,nSet,'single'));
reconSoS = zeros(nread,npe1,npe2,nSet,'single');
if ~isempty(combinePars)
    dataCombined = complex(zeros(nread,npe1,npe2,nSet,'single'));
end
...
parfor iS = 1:nread 
   ...
   %%
            reconSoS(iS,:,:,iSet) = reshape(thisImage,[1 npe1 npe2]);
            grappaRecon_1DFFT(iS,:,:,:,iSet) = reshape(permute(thisGrapRecon,[3 1 2]),[1 nc npe1 npe2]);
