...
load brain_alias_8ch % 320*256*8
DATA = DATA/max(max(max(abs(ifft2c(DATA))))) + eps; % normalize
...
DATAc = DATA.*mask; % undersampled k-space
calib = crop(DATAc,[sx,ncalib,Nc]); % calibration region
...
imshow3(abs(im_full),[],[1,Nc]); 
title('magnitude of physical coil images');
colormap((gray(256))); colorbar;

subplot(2,1,2); imshow3(angle(im_full),[],[1,Nc]); 
...
subplot(2,1,2); imshow3(angle(im),[],[1,Nc]); 
title('phase of physical coil images');
colormap('default'); colorbar;
...
hold on, 
plot([1:ksize(1)*ksize(2)*Nc],S(1)*eigThresh_im,'r-','LineWidth',2);
...
[M,W] = kernelEig(k(:,:,:,1:idx),[sx,sy]); % M = eigenvectors, W = eigenvalues
...
disp('Performing ESPIRiT reconstruction from 2 maps')
tic; [reskESPIRiT_full, resESPIRiT_full] = cgESPIRiT(DATA,ESP, nIterCG, 0.01,DATAc*0); toc
...
figure; imshow(cat(2, abs(resSENSE), sos(resESPIRiT)), []);

imx([sos(im_full), abs(resSENSE); sos(im_full), sos(resESPIRiT)], 11);

imx([sos(im_full), sos(resESPIRiT_full)], 12);
